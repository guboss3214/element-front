...
const CapacitorNode: FC<Omit<NodeProps, 'nodeType' | 'label'>> = ({
  id,
  onDragStart,
}) => {
  return (
    <div
      id={id}
      draggable="true"
      onDragStart={(e) => onDragStart(e, 'capacitor')}
    ...
    >
      ...
      <BsBatteryHalf className="text-green-400" size={22} />
      <span className="text-sm font-medium">Конденсатор</span>
      <div className="w-3 h-3 bg-gray-400 rounded-full border-2 border-gray-600"></div>
    </div>
  )
}
